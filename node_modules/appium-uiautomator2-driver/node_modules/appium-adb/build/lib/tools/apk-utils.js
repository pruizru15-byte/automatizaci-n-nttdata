...
async function cacheApk(apkPath, options = {}) {
    ...
    if (remoteCachedFiles.some((x) => toHash(x) === appHash)) {
        ...
        logger_js_1.log.info(`The upload of '${node_path_1.default.basename(apkPath)}' (${support_1.util.toReadableSizeString(size)}) ` +
...
async function install(appPath, options = {}) {
    ...
    try {
        ...
        logger_js_1.log.info(`The installation of '${node_path_1.default.basename(appPath)}' took ${timer.getDuration().asMilliSeconds.toFixed(0)}ms`);
