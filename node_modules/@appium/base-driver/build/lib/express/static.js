...
let STATIC_DIR = node_path_1.default.resolve(__dirname, '..', '..', '..', 'static');
exports.STATIC_DIR = STATIC_DIR;
if (lodash_1.default.isNull(node_path_1.default.resolve(__dirname).match(/build[/\\]lib[/\\]express$/))) {
    // in some contexts we are not in the build directory,
    // so we don't want to go back the extra level
    exports.STATIC_DIR = STATIC_DIR = node_path_1.default.resolve(__dirname, '..', '..', 'static');
}
...
async function getTemplate(templateName) {
    let content = await support_1.fs.readFile(node_path_1.default.resolve(STATIC_DIR, 'test', templateName));
